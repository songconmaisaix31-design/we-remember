...
1. 使用公开演示家庭码 `DEMO-HOME` 进入。
2. 选择预设头像或上传本地头像。
3. 通过文字或浏览器语音向 Agent 描述安排。
4. Agent 先生成草稿，用户明确确认后才写入本页日程。
5. 确认后生成按家庭成员展示的本地通知回执。
...
本地原型的核心路径已经完整：进入家庭、选择身份、向 Agent 描述安排、确认草稿、查看时间表、查看成员与通知回执、进入连接中心。当前可交付的是经过响应式和浏览器验证的本地交互原型，不是具备真实账号、日历、消息渠道和持久化的生产系统。
